# Copyright (c) 2023 Airbyte, Inc., all rights reserved.

import json

from datetime import datetime
from typing import Any, Dict


class ConfigBuilder:
    def __init__(self) -> None:
        self._config: Dict[str, Any] = {}            

    def default(self) -> "ConfigBuilder":
        self.with_project_id("glb-clientadvisor-prod")
        self.with_credentials_json(json.dumps({
            "type": "service_account",
            "project_id": "XXXXXXXX",
            "private_key_id": "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX",
            "private_key": "-----BEGIN PRIVATE KEY-----\\nXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n-----END PRIVATE KEY-----\n",
            "client_email": "XXXXXX@XXXXXXX.iam.gserviceaccount.com",
            "client_id": "000000000000000000000",
            "auth_uri": "https://accounts.google.com/o/oauth2/auth",
            "token_uri": "https://oauth2.googleapis.com/token",
            "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
            "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/XXXXXXX%40XXXXXXXXX.iam.gserviceaccount.com",
            "universe_domain": "googleapis.com"
        }))
        self.with_replication_method("Standard")
        return self

    def with_project_id(self, project_id: str) -> "ConfigBuilder":
        self._config["project_id"] = project_id
        return self

    def with_credentials_json(self, credentials_json: str) -> "ConfigBuilder":
        self._config["credentials_json"] = credentials_json
        return self

    def with_replication_method(self, replication_method: str) -> "ConfigBuilder":
        self._config["replication_method"] = {"method": replication_method}
        return self
    
    def with_filtered_stream(self, stream_name: str, parent_stream_name: str, where_clause: str) -> "ConfigBuilder":
        self._config.setdefault("streams", []).append({
            "name": stream_name,
            "parent_stream": parent_stream_name,
            "where_clause": where_clause
        })
        return self

    def build(self) -> Dict[str, Any]:
        return self._config
